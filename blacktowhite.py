...
def onAppStart(app):
    ...
    app.canvasGrid = [[0 for col in range(app.cols)]
...
def redrawAll(app):
    ...
    makeBackgroundWhite(app)


def makeBackgroundWhite(app):
    for row in range(app.rows):
        for col in range(app.cols):
            if app.canvasGrid[row][col] > 0:
                drawRect(col * app.gridSize, row * app.gridSize, app.gridSize,
                         app.gridSize, fill='white', opacity=app.canvasGrid[row][col])
            else:
                drawRect(col * app.gridSize, row * app.gridSize, app.gridSize,
                         app.gridSize)
...
def updateBoids(app):
    ...
    for i, boid in enumerate(app.boids):
        ...
        # updating background
        currCol = int(boid['x'] // app.gridSize)
        currRow = int(boid['y'] // app.gridSize)

        if 0 <= currCol < app.cols and 0 <= currRow < app.rows:
            if app.canvasGrid[currRow][currCol] < 100:
                app.canvasGrid[currRow][currCol] += 2
